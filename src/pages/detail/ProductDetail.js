...
const ProductDetail = () => {
	...
	const optionsMarkdown = {
		overrides: {
			ul: {
				component: (props) => (
					<ul style={{ listStyle: 'disc', paddingLeft: '1em' }} {...props} />
				),
			},
			li: {
				component: (props) => <li style={{ marginTop: '0.5em' }} {...props} />,
			},
		},
	}
	if (!detailLaptop) return null
	return (
		<Layout>
			<div className="detail_container">
							...
							{detailLaptop.sale_out ? (
								...
								<div className="promotion_price-product">
									...
									{detailLaptop?.price_promotion > 0 && (
										<p className="price">
											{formatNumber(detailLaptop?.price_promotion)}₫
										</p>
									)}
								</div>
							)}
				...
				<div className="description">
					<div className="description-left">
						{detailLaptop?.description ? (
							<div className="border-view">
								<p className="border-title">Đặc điểm nổi bật</p>
								<Markdown options={optionsMarkdown} className="content_view">
									{detailLaptop?.description || ''}
								</Markdown>
							</div>
						) : null}
